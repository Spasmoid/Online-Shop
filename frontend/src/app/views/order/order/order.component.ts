...
export class OrderComponent implements OnInit {
  ...
  createOrder() {
    if (this.orderForm.valid && this.orderForm.value.firstName && this.orderForm.value.lastName && this.orderForm.value.phone && this.orderForm.value.paymentType && this.orderForm.value.email) {
        ...
        .subscribe({
          ...
          error: (errorResponse: HttpErrorResponse) => {
            if (errorResponse.error && errorResponse.message) {
              ...
              this._snackbar.open('Не удалось создать заказ');
            }
          }
        });
    } else {
